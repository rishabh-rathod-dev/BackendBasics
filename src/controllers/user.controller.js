...
const generateAccessandRefreshToken = async (userId) => {
  try {
    ...
    // Debug: log generated token and existing stored token before saving
    console.log("[token-debug] generated refreshToken:", refreshToken);
    console.log(
      "[token-debug] previous stored refreshToken:",
      user?.refreshToken
    );

    // store and persist the new refresh token
    user.refreshToken = refreshToken;
    await user.save({ validateBeforeSave: false });

    // Debug: verify saved token (re-fetch from DB to ensure persistence)
    const persisted = await User.findById(userId).select("refreshToken");
    console.log(
      "[token-debug] saved refreshToken (from DB):",
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Debug logs to compare incoming token vs stored token
    ...
    // console.log(accessToken, refreshAccessToken, "Access and refresh token");
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    req.user?._id,
    { $set: { coverImage: coverLocalPath.url } },
    { new: true }
  ).select("-password");
